...
export const sendDirectMessage = async (
    ...
    try {
        ...
        if (!friendship) {
            return res
                .status(403)
                .json({ error: "Can only send messages to friends" });
        }
        ...
        const messageWithSender = await DirectMessage.findByPk(
            directMessage.id,
            {
                include: [
                    {
...
export const getConversation = async (
    ...
    try {
        ...
        if (!friendship) {
            return res
                .status(403)
                .json({ error: "Can only view conversations with friends" });
        }
                ...
                return {
                    ...
                    latestMessage: latestMessage
                        ? {
                              ...
                              senderUsername: (latestMessage as any).sender
                                  .username,
                          }
                        : null,
                };
